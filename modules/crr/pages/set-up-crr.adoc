...
By setting up cross-region replication (CRR), you set up Disaster Recover (DR) clusters that sync with the primary cluster.
...
However, you can skip <<_enable_crr_on_the_dr_cluster, Step 2.3: Enable CRR on the DR cluster>>, because CRR will have already been enabled.
